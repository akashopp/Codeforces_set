...
*/
ll dp[200001][2];
ll f(int i, bool flag, int num, int &s, vi &vec, int n){
  if(i == n-1){
    return vec[i]*1ll*num;
  }
  if(dp[i][flag] != -1) return dp[i][flag];
  ll mini1 = min(s, vec[i]-s);
  mini1 = max(0ll, mini1);
  ll mini2 = vec[i] - mini1;
  ll a = num*mini1 + f(i+1, 0, mini2, s, vec, n);
  ll b = num*mini2 + f(i+1, 1, mini1, s, vec, n);
...
void solve(){
  ...
  cout<<f(1,0,vec[0],s,vec,n)<<nline;
}
signed main(){
#ifndef ONLINE_JUDGE
