...
// JAI SHREE RAM
// Most frequent element in a range query
...
/*---------------------------------------------------------------------------------------------------------------------------*/
const int N = 3e5 + 5;
int n, k, vec[N], sq = 550, freq[N], freqOfFreq[N], maxi;
void add(int x){
  int prev = freq[x];
  freq[x]++;
  int cur = freq[x];
  freqOfFreq[prev]--, freqOfFreq[cur]++;
...
void remove(int x){
  ...
  int cur = freq[x];
  freqOfFreq[prev]--, freqOfFreq[cur]++;
  while(freqOfFreq[maxi] == 0) maxi--;
}
void solve(){
  cin>>n>>k;
  vector<pair<pi,int>>q(k);
  for0(i,n) cin>>vec[i];
  for0(i,k){
  ...
  for0(i,k){
    ...
    int total = r - l + 1;
    int rem = total - maxi;
    if(total <= 2*rem+1) ans[ind] = 1;
    else ans[ind] = total - 2*rem;
  }
