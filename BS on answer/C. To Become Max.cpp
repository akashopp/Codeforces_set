...
NOTES: 
  ...
  -> 
*/
bool f(int i, int mid, vi &vec, int rem){
  if(i == sz(vec)) return 0;
  if(vec[i] >= mid) return 1;
  int x = mid - vec[i];
  if(x > rem) return 0;
  return f(i+1, mid-1, vec, rem-x);
}
void solve(){
  int n, k; cin>>n>>k;
  vi vec(n);
  ...
  int ans = vec[n-1];
  for0(i,n-1){
    int low = vec[i], high = vec[i]+k;
    while(low <= high){
      int mid = low + (high-low)/2;
      if(f(i, mid, vec, k)) ans = max(ans, mid), low = mid+1;
      else high = mid-1;
    }
  }
  cout<<ans<<nline;
